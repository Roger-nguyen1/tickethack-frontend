  ...
  for (let i = 0; i < document.querySelectorAll(".delete").length; i++) {
    document
      ...
      [i].addEventListener("click", function () {
        fetch(
          `https://tickethack-backend-nine-gules.vercel.app/cart/${this.id}`,
          { method: "DELETE" }
        )
            ...
            if (data.result) {
              ...
              if (document.querySelectorAll(".delete").length === 0) {
                ...
                document.querySelector("#total").textContent =
                  data.bookings.reduce((acc, { trip }) => acc + trip.price, 0);
              }
            }
          });
      });
...
// Get cart
fetch("https://tickethack-backend-nine-gules.vercel.app/cart")
  .then((response) => response.json())
  .then((data) => {
    if (data.result) {
...
document.querySelector("#purchase").addEventListener("click", function () {
  fetch("https://tickethack-backend-nine-gules.vercel.app/bookings", {
    method: "PUT",
  })
    .then((response) => response.json())
    .then((data) => {
